...
const ClientesFornecedores = () => {
  const [items, setItems] = useState<ClienteFornecedor[]>(initialClientesFornecedores);
  const [search, setSearch] = useState("");
  const [tipoFilter, setTipoFilter] = useState("todos");
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editItem, setEditItem] = useState<ClienteFornecedor | null>(null);
  const [deleteOpen, setDeleteOpen] = useState(false);
  const [deleteId, setDeleteId] = useState<number | null>(null);

  const emptyItem: ClienteFornecedor = { id: 0, nome: "", tipo: "cliente", documento: "", contato: "" };

  const handleNew = () => { setEditItem({ ...emptyItem, id: Date.now() }); setDialogOpen(true); };
  const handleEdit = (item: ClienteFornecedor) => { setEditItem({ ...item }); setDialogOpen(true); };
  const handleSave = () => {
    ...
    <div className="space-y-6 animate-fade-in">
              ...
              {filtered.map(item => (
                <tr key={item.id} className="border-b border-border/50 hover:bg-muted/20 transition-colors">
                  ...
                  <td className="px-5 py-3.5 text-sm text-center">
                    <span className={`text-[11px] font-semibold capitalize px-2.5 py-1 rounded-full ${item.tipo === "cliente" ? "bg-success/10 text-success" : "bg-warning/10 text-warning"}`}>
                      {item.tipo}
                    </span>
                  </td>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader><DialogTitle>{editItem && items.some(i => i.id === editItem.id) ? "Editar" : "Novo"} Cadastro</DialogTitle></DialogHeader>
          {editItem && (
            <div className="space-y-4 pt-2">
              ...
              <div className="space-y-2">
                <Label>Tipo</Label>
                <Select value={editItem.tipo} onValueChange={v => setEditItem({ ...editItem, tipo: v as "cliente" | "fornecedor" })}>
      ...
      <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader><DialogTitle>Excluir Cadastro</DialogTitle></DialogHeader>
